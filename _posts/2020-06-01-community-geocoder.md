...
この API は、経産省の [「IMI コンポーネントツール」](https://info.gbiz.go.jp/tools/imi_tools/) をフォークしたライブラリを使用しています。
...
* たとえば「京都市中京区寺町通御池上る上本能寺前町488番地」のように「通り名」があるなどの理由で住所の判定ができない場合、その上の階層、この例では「京都市中京区」の緯度経度を返します。
* 政令指定都市では、「大阪市中央区」などのように「区」まで入力して頂く必要があります。
* 都道府県名のみによる検索、ランドマーク（たとえば「スカイツリー」）による検索には対応していません。
