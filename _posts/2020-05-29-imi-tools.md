...
まず、ご自身のプロジェクト内に `bin/get-imi-tools.sh` というファイル名で以下のシェルスクリプトを設置してください。
...
このシェルスクリプトでは、 `imi-enrichment-address` をダウンロードして `imi-tools/imi-enrichment-address` というディレクトリ内に設置しています。
